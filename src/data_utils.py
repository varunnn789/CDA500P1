...
#The above line convert the parent directory to system path so 
#python is able to import from the parent directory as well
...
#Filters the data based on duration, total_amount, location_ids, and date ranges
...
#Returns the combined taxi data for entire year (or a list of months within the year if specified)
...
def fill_missing_rides_full_range(df, hour_col, location_col, rides_col):
    ...
    return merged_df
#Fills missing location and hour combination entries with 0 values 
#(used within transform_raw_data_into_ts_data)
...
#Takes the taxi data file and truncates to hour, aggregates by location-hour combination
#and calls fill_missing_rides_full_range to fill missing entries with 0
...
#Converts the processed data from earlier function to generate attirbutes and target based on a 'window lead'
...
def transform_ts_data_info_features_and_target(
    ...
    return features, targets
#Seems the function got repeated?
...
#Splits the input dataset into training and test test based on a cut-off date.
#X_train = features before cutoff
